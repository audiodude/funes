...
async fn check_reports_without_writing() {
    ...
    // OMP indexing normally writes graph sidecars. A dry run must not, and an
    // incomplete parse must not be reported as a clean check of the retained prefix.
    let sources = tempfile::tempdir().unwrap();
    let omp = sources.path().join("omp.jsonl");
    let complete = concat!(
        "{\"type\":\"session\",\"version\":3,\"id\":\"dry-run\",\"cwd\":\"/tmp\"}\n",
        "{\"type\":\"message\",\"id\":\"m\",\"parentId\":null,\"message\":{\"role\":\"user\",\"content\":\"retained\"}}\n"
    );
    std::fs::write(&omp, complete).unwrap();
    let check_omp = || {
        funes::commands::index::check(&omp, false, None, Some(funes::traces::harness::Harness::Omp)).unwrap()
    };
    assert!(check_omp().is_clean());
    assert_eq!(std::fs::read_dir(home.path()).unwrap().count(), 0);
    std::fs::write(&omp, format!("{complete}{{invalid}}\n")).unwrap();
    let incomplete = check_omp();
    assert!(!incomplete.is_clean());
    assert_eq!(incomplete.rejected, 1);
