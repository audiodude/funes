...
/// The agent `sketch` format: a header stating what the digest covers and what it cost, then each
/// selected passage with the `→ get` that reads the turns around it. A clamped request and a
/// shortened passage are stated, never applied silently. Byte-stable.
